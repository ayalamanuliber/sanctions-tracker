    ...
    <main className={shell.main}>
      ...
      <form className={`${shell.card} ${styles.searchPanel}`} method="get">
        <div className={styles.searchRow}>
          ...
          <div className={styles.field}><label htmlFor="severity">Severity</label><select id="severity" name="severity" defaultValue={query.severity}><option value="">All levels ({LEGAL_RISK_CASES.length.toLocaleString()})</option>{FILTER_COUNTS.severities.map((item) => <option key={item.value} value={item.value}>{optionLabel(item)}</option>)}</select></div>
